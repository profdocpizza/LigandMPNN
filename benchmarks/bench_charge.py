...
Writes benchmarks/results/charge_sweep.csv, charge_baseline.csv, and the
all-model exact/+-4 tolerance sweep in charge_tolerance.csv.
"""
...
def tolerance_sweep():
    """Measure how a +-4 e window changes achieved charge across the sweep."""
        ...
        for target in TARGETS:
            for tol in (0, 4):
                ...
                print(
                    ...
                    f"inside {inside}/{len(got)}, "
                    f"achieved {min(got):+.0f}..{max(got):+.0f}",
                    flush=True,
                )
...
def main():
    ...
    with open(os.path.join(OUT, "charge_baseline.csv"), "w", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=list(base_rows[0]), lineterminator="\n")
        w.writeheader()
