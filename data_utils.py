...
def write_full_PDB(
    ...
    """
    # restype_1to3 is already global
    # restype_name_to_atom14_names is now global

    # Need a local restype_INTtoSTR for this function's logic if S is int
    # Or, assume S is already 1-letter codes, or convert S using global restype_int_to_str
    # The existing code uses S as integer array and converts here.
    # Let's keep a local INTtoSTR or use the global one.
    # Global restype_int_to_str is available.

    S_str = [restype_1to3[AA] for AA in [restype_int_to_str[AA_idx] for AA_idx in S]]
